...
/**Average volume of the various solar system objects**/
SELECT AVG(volume) FROM solar_system_objects;

/**Which solar system object has the highest mass?**/
SELECT MAX(mass) FROM solar_system_objects;

/**Which solar system objects are round?**/
SELECT * FROM solar_system_objects WHERE shape IN ("round");

/**What if we needed a column detailing which objects had the highest/lowest mass and how many objects were in each category?**/
SELECT COUNT(*),
CASE
    WHEN mass > 10000 THEN "very high mass"
    WHEN mass > 4000 THEN "high mass"
    WHEN mass < 1000 THEN "low mass"
    WHEN mass < 300 THEN "very low mass"
END AS "mass_comparison" FROM solar_system_objects
GROUP BY "mass_comparison";

/** 4g max gravity. 0.2 min gravity. which objects have a safe surface gravity for humans? **/
SELECT body FROM solar_system_objects WHERE (surface_gravity BETWEEN 0.20 AND 4);

/**How many objects have a safe surface gravity for humans?**/
SELECT COUNT(*),
CASE
    WHEN surface_gravity <= 4 THEN "safe"
    WHEN surface_gravity > 4 THEN "too high"
    WHEN surface_gravity < 0.20 THEN "too low"
END AS "safety for humans" FROM solar_system_objects
GROUP BY "safety for humans";

/** smaller planets**/
SELECT body, volume FROM solar_system_objects desc
GROUP BY volume HAVING volume < 100; 

/** bigger planets**/
SELECT body, volume FROM solar_system_objects desc
GROUP BY volume HAVING volume > 100;

/** safer terrestrial planets for humans based on surface gravity**/
SELECT body FROM solar_system_objects WHERE (type_of_object = "planet (terrestrial)" AND surface_gravity BETWEEN 0.20 AND 4); 

/**satellite planets**/
SELECT body, type_of_object FROM solar_system_objects WHERE type_of_object LIKE "%satellite%"
